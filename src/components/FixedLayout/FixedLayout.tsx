...
class FixedLayout extends React.Component<FixedLayoutProps & DOMProps & PanelContextProps, FixedLayoutState> {
  ...
  onViewTransitionStart: EventListener = (e: CustomEvent<TransitionStartEventDetail>) => {
    ...
    // если переход назад - анимация только у панели с которой уходим (second), и подстраиваться под скролл надо только на ней
    if (fromPanelHasScroll || toPanelHasScroll && this.canTargetPanelScroll && this.props.panel === 'second') {
      this.setState({
        position: 'absolute',
